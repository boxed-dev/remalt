...
export const YouTubeNode = memo(
  ...
  const isActive = activeNodeId === id;
    ...
    type NativeEventWithStop = Event & {
    ...
    const fetchVideoTitle = useCallback(async (videoUrl: string) => {
      try {
        const response = await fetch(
          `/api/youtube/metadata?url=${encodeURIComponent(videoUrl)}`,
          {
            credentials: "include",
          }
        );
    ...
    const processVideoUrl = useCallback(
      async (videoUrl: string, silent = false) => {
        ...
        const nodeData =
          node?.type === "youtube" ? (node.data as YouTubeNodeData) : undefined;
        ...
        const shouldFetchTitle =
          !nodeData?.title ||
          nodeData.title === "YouTube Video" ||
          existingVideoId !== videoId;
        ...
        if (hasTranscript) {
          console.log(
            `[YouTubeNode] Using cached transcript for video: ${videoId}`
          );
          return;
        }
    ...
    const processUrl = useCallback(
      async (inputUrl: string, silent = false) => {
    ...
    useEffect(() => {
      ...
      if (node?.type === "youtube") {
        ...
        if (
          nodeData.mode === "channel" &&
          nodeData.channelLoadStatus &&
          nodeData.channelLoadStatus !== "loading"
        ) {
        ...
        if (
          nodeData.videoId &&
          nodeData.transcriptStatus &&
          nodeData.transcriptStatus !== "loading"
        ) {
          processedUrlRef.current = data.url;
          return;
        }
    ...
    const downloadAllTranscripts = (
      ...
      const selectedVideos =
        data.channelVideos?.filter((v) => v.selected && v.transcript) || [];
      if (selectedVideos.length === 0) return;

      const allTranscripts = selectedVideos
        .map((v) => `=== ${v.title} ===\n\n${v.transcript}\n\n`)
        .join("\n");

      const blob = new Blob([allTranscripts], {
        type: "text/plain;charset=utf-8",
      });
      const urlObject = URL.createObjectURL(blob);
      const link = document.createElement("a");
    ...
    const toggleVideoSelection = async (videoId: string) => {
      ...
      if (
        ...
        console.log(
          `[YouTubeNode] Fetching transcript for channel video: ${videoId}`
        );
        const result = await fetchTranscript(videoUrl);
        console.log(
          `[YouTubeNode] Transcript fetch result for ${videoId}:`,
          result.status,
          result.method
        );
        ...
        const currentVideos =
          (currentState?.type === "youtube"
            ? (currentState.data as YouTubeNodeData).channelVideos
            : undefined) || [];
    ...
    const TranscriptStatus = () => {
      ...
      switch (data.transcriptStatus) {
        ...
        case "unavailable":
          return (
            <div
              className="flex items-center gap-1 text-[10px] text-[#B45309] font-medium"
              title="This video doesn't have captions. Try a video with CC enabled."
            >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between bg-[#F8FAFC] -mx-3 px-3 py-2 rounded-lg">
              <div className="text-[12px] font-medium text-[#0F172A]">
                {data.channelVideos.length} Videos ({selectedVideosCount}{" "}
                selected)
              </div>
              <button
                onClick={(e) => {
