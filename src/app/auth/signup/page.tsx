...
export default function SignUpPage() {
  ...
  const handleLinkedInSignUp = async () => {
    ...
    try {
      ...
      if (signInError) {
        console.error('[LinkedIn SignUp] OAuth error:', signInError);
        setError(signInError.message);
        setLoading(false);
      } else {
        console.log('[LinkedIn SignUp] OAuth initiated successfully');
      }
    } catch (err: any) {
      console.error('[LinkedIn SignUp] Exception:', err);
      setError(err.message || 'An unexpected error occurred');
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#FAFBFC] px-4">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl border border-[#E8ECEF] p-8 shadow-[0_2px_8px_rgba(0,0,0,0.04)] text-center">
            ...
            <Link
              href="/auth/signin"
              className="inline-block px-6 py-2.5 bg-[#007AFF] text-white rounded-lg font-medium text-[14px] hover:bg-[#0051D5] transition-all"
            >
              Back to Sign In
            </Link>
          </div>
