...
export function SaveStatusIndicator() {
  ...
  const { Icon, text, textClassName, iconClassName, tooltip, showPulse } = useMemo(() => {
    switch (status) {
      case 'saving':
        return {
          Icon: Cloud,
      ...
      case 'error':
        return {
          ...
          iconClassName: '',
          tooltip: saveError || 'Failed to save. Your changes are stored locally.',
          showPulse: false,
        };
      case 'saved':
        const relativeTime = lastSaved ? formatRelativeTime(lastSaved) : '';
        const fullTime = lastSaved ? new Date(lastSaved).toLocaleString(undefined, {
          dateStyle: 'medium',
          timeStyle: 'short'
        }) : '';
